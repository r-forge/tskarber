...
# Hamilton. It does not seem to add much to the execution time, even
# though if there's more than one nondecreasing point it will iterate
# until it reaches machine epsilon.
    is.smoothed <- FALSE
    i <- 1L
...
# Linearly interpolate points where the lines p=0 and p=1 meet the trim.
# NOTE: From this point on, there is a variable called L. This makes the parts
# with integer literals denoted by L kind of hard to read.
    ...
    trimx <- keepers$x
    trimp <- keepers$p
# There's some voodoo with the approx interpolation function here... it doesn't
# work right if there are mulitple doses with the same p-values, and ordered,
# min, max, etc. don't do what I want. Subscripting seemed more efficient than
# building a function to pass to approx to make it work right.
